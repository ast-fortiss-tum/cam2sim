...
with the ability to override the cam2sim auto-detection and load arbitrary
external models (the old --gs_configs / --gs_data_root style).

USAGE MODES
-----------

1) cam2sim mode (default) - reads from:
       ...
       data/data_for_gaussian_splatting/<BAG>/outputs/splatfacto_split_N/.../config.yml
       data/data_for_gaussian_splatting/<BAG>/outputs/splatfacto_split_N/.../utm_to_nerfstudio_transform.json
       data/data_for_gaussian_splatting/<BAG>/frame_positions_split_N_1_of_K.txt

   No extra arguments needed (or just --skip_calibration / --no_save).

2) Legacy external-models mode - same script, but with explicit override:
       --gs_configs CFG1.yml CFG2.yml CFG3.yml
       --gs_data_root datasets/sunny
       [--utm_transforms TF1.json TF2.json TF3.json]   (optional, auto-resolved
                                                        from each config dir)
       [--map sunny_map]      (overrides the xodr path under processed_dataset)
       [--dataset sunny]      (overrides the camera.json and trajectory path
                               to read from the legacy datasets/<dataset>/ tree)

   This is identical in spirit to the old replay_with_gs_nerfstudio.py
   command-line, so you can re-run pre-cam2sim models without rebuilding
   anything.

PHASE 1: 4-panel calibration GUI (CARLA, GS free cam, original training image,
         GS rendered from training pose).
PHASE 2: Replay - drive trajectory with CARLA + GS side by side.

Run with --skip_calibration to jump straight to phase 2.

Coordinate chain (per split):
    CARLA local coords -> UTM (inverse XODR projection) -> Nerfstudio (similarity)
"""
...
# =============================================================================
#  DEFAULT CONFIG (cam2sim mode)
# =============================================================================

BAG_NAME = "reference_bag"

DEFAULT_XODR_FILE = os.path.join(
...
DEFAULT_GS_DATA_ROOT = os.path.join(
...
def auto_detect_splits_cam2sim(gs_outputs_dir, gs_data_root):
    """
    Look for splatfacto splits under the cam2sim layout:
...
def build_split_configs_from_external(gs_configs, gs_data_root,
                                       utm_transforms=None,
                                       frame_positions_dir=None):
    """
    Build split configs from explicit external paths (legacy --gs_configs style).

    For each config:
      - utm_transform: from --utm_transforms, else <cfg_dir>/utm_to_nerfstudio_transform.json
      - split_num:     parsed from "split<N>" in the config path; fallback to position
      - frame_positions: looked up in frame_positions_dir (defaults to gs_data_root)
                         using common legacy naming patterns
    """
    ...
    if len(utm_transforms) != len(gs_configs):
        print(f"[ERROR] Got {len(gs_configs)} configs but "
              f"{len(utm_transforms)} transforms")
        sys.exit(1)

    fp_dir = frame_positions_dir or gs_data_root

    split_configs = []
    for i, (cfg_path, tf_path) in enumerate(zip(gs_configs, utm_transforms)):
        # Try to recover split number from "split<N>" or "split_<N>" in path
        split_num = None
        match = re.search(r"split[_-]?(\d+)", cfg_path)
        if match:
            split_num = int(match.group(1))
        if split_num is None:
            # Fall back to positional ordering, 1-based
            split_num = i + 1

        # Resolve frame_positions file: try multiple legacy patterns
        frame_positions = None
        candidates = []

        # Pattern A: frame_positions_split_<N>_<anything>.txt
        try:
            for fname in os.listdir(fp_dir):
                if (fname.startswith(f"frame_positions_split_{split_num}_")
                        and fname.endswith(".txt")):
                    candidates.append(os.path.join(fp_dir, fname))
        except FileNotFoundError:
            pass

        # Pattern B: legacy fixed name frame_positions_split_<N>_1_of_2.txt
        for legacy_name in [
            f"frame_positions_split_{split_num}_1_of_2.txt",
            f"frame_positions_split_{split_num}_1_of_3.txt",
            f"frame_positions_split_{split_num}.txt",
            "frame_positions.txt",
        ]:
            p = os.path.join(fp_dir, legacy_name)
            if os.path.exists(p):
                candidates.append(p)

        if candidates:
            frame_positions = candidates[0]
            print(f"[INFO] Using frame positions for split_{split_num}: "
                  f"{os.path.basename(frame_positions)}")
        else:
            print(f"[WARN] No frame positions file found for split_{split_num} "
                  f"in {fp_dir}")
        ...
        split_configs.append({
            ...
            "frame_positions": frame_positions,
            "data_root": gs_data_root,
            "run_name": Path(cfg_path).parts[-2] if len(Path(cfg_path).parts) >= 2 else "external",
        })
        print(f"[INFO] External split_{split_num}: {cfg_path}")

    split_configs.sort(key=lambda s: s["split_num"])
...
# =============================================================================
#  PATH RESOLUTION (cam2sim default vs legacy override)
# =============================================================================

def resolve_input_paths(args):
    """
    Decide which input paths to use, with three layers of priority:

      1) Explicit --xodr_file / --trajectory_file / --camera_config_file
         override everything (recommended for legacy models).
      2) --map / --dataset triggers legacy directory conventions
         (data/processed_dataset/<map>/maps, maps/<map>/, datasets/<dataset>/).
      3) Falls back to the cam2sim reference_bag defaults.

    Returns a dict with: xodr_file, trajectory_file, camera_config_file,
    gs_data_root, gs_outputs_dir.
    """
    # ---- xodr ----
    if args.xodr_file:
        if not os.path.exists(args.xodr_file):
            print(f"[ERROR] --xodr_file does not exist: {args.xodr_file}")
            sys.exit(1)
        xodr_file = args.xodr_file
    elif args.map:
        cam2sim_xodr = os.path.join(
            PROJECT_ROOT, "data", "processed_dataset",
            args.map, "maps", "map.xodr"
        )
        legacy_xodr = os.path.join("maps", args.map, "map.xodr")
        if os.path.exists(cam2sim_xodr):
            xodr_file = cam2sim_xodr
        elif os.path.exists(legacy_xodr):
            xodr_file = legacy_xodr
        elif os.path.exists(DEFAULT_XODR_FILE):
            print(f"[WARN] No xodr under cam2sim/legacy {args.map}, "
                  f"falling back to reference_bag default.")
            xodr_file = DEFAULT_XODR_FILE
        else:
            print(f"[ERROR] Could not find xodr under any of:")
            print(f"        {cam2sim_xodr}")
            print(f"        {legacy_xodr}")
            print(f"        {DEFAULT_XODR_FILE}")
            sys.exit(1)
    else:
        xodr_file = DEFAULT_XODR_FILE
    ...
    if args.trajectory_file:
        if not os.path.exists(args.trajectory_file):
            print(f"[ERROR] --trajectory_file does not exist: {args.trajectory_file}")
            sys.exit(1)
        trajectory_file = args.trajectory_file
    elif args.dataset:
        legacy_traj = os.path.join(
            "datasets", args.dataset,
            "trajectory_positions_rear_odom_yaw.json"
        )
        if args.map:
            map_traj = os.path.join(
                "maps", args.map,
                "trajectory_positions_rear_odom_yaw.json"
            )
        else:
        ...
        if os.path.exists(legacy_traj):
            trajectory_file = legacy_traj
        elif map_traj and os.path.exists(map_traj):
            trajectory_file = map_traj
        elif os.path.exists(DEFAULT_TRAJECTORY_FILE):
            print(f"[WARN] No trajectory under legacy dataset {args.dataset}, "
                  f"falling back to reference_bag default.")
            trajectory_file = DEFAULT_TRAJECTORY_FILE
        else:
            print(f"[ERROR] No trajectory file. Tried:")
            print(f"        {legacy_traj}")
            if map_traj:
                print(f"        {map_traj}")
            print(f"        {DEFAULT_TRAJECTORY_FILE}")
            sys.exit(1)
    else:
        trajectory_file = DEFAULT_TRAJECTORY_FILE

    # ---- camera config ----
    if args.camera_config_file:
        if not os.path.exists(args.camera_config_file):
            print(f"[ERROR] --camera_config_file does not exist: "
                  f"{args.camera_config_file}")
            sys.exit(1)
        camera_config_file = args.camera_config_file
    elif args.dataset:
        legacy_cam = os.path.join("datasets", args.dataset, "camera.json")
        if os.path.exists(legacy_cam):
            camera_config_file = legacy_cam
        elif os.path.exists(DEFAULT_CAMERA_CONFIG_FILE):
            print(f"[WARN] No camera.json under legacy dataset {args.dataset}, "
                  f"falling back to reference_bag default.")
            camera_config_file = DEFAULT_CAMERA_CONFIG_FILE
        else:
            print(f"[ERROR] No camera.json. Tried:")
            print(f"        {legacy_cam}")
            print(f"        {DEFAULT_CAMERA_CONFIG_FILE}")
            sys.exit(1)
    else:
        camera_config_file = DEFAULT_CAMERA_CONFIG_FILE

    # ---- GS data root ----
    if args.gs_data_root:
        gs_data_root = args.gs_data_root
        gs_outputs_dir = os.path.join(gs_data_root, "outputs")
    else:
        gs_data_root = DEFAULT_GS_DATA_ROOT
        gs_outputs_dir = os.path.join(gs_data_root, "outputs")
...
def main():
    parser = argparse.ArgumentParser(
        description="Replay with Nerfstudio-trained GS (multi-split). "
                    "Defaults to cam2sim layout under data/, with legacy "
                    "override via --gs_configs."
    )
    ...
    # Legacy override flags (mirror replay_with_gs_nerfstudio.py)
    parser.add_argument("--map", type=str, default=None,
                        help="Map name (overrides default reference_bag xodr). "
                             "Tried under data/processed_dataset/<map>/maps/ and "
                             "maps/<map>/.")
    parser.add_argument("--dataset", type=str, default=None,
                        help="Dataset name (overrides trajectory + camera config "
                             "to read from datasets/<dataset>/).")
    parser.add_argument("--gs_configs", type=str, nargs="+", default=None,
                        help="Explicit list of nerfstudio config.yml paths. "
                             "Bypasses cam2sim auto-detection.")
    parser.add_argument("--gs_data_root", type=str, default=None,
                        help="Data root used as cwd when calling eval_setup. "
                             "Required with --gs_configs in most legacy setups.")
    parser.add_argument("--utm_transforms", type=str, nargs="+", default=None,
                        help="Optional explicit list of UTM transform JSONs, "
                             "one per --gs_configs. If omitted, each is "
                             "auto-resolved as <cfg_dir>/utm_to_nerfstudio_transform.json.")

    # Explicit file overrides (highest priority - used for legacy models)
    parser.add_argument("--xodr_file", type=str, default=None,
                        help="Explicit path to a map.xodr file. Highest priority.")
    parser.add_argument("--trajectory_file", type=str, default=None,
                        help="Explicit path to trajectory_positions_rear_odom_yaw.json.")
    parser.add_argument("--camera_config_file", type=str, default=None,
                        help="Explicit path to camera.json.")
    ...
    CAMERA_CONFIG_FILE = paths["camera_config_file"]
    GS_DATA_ROOT = paths["gs_data_root"]
    GS_OUTPUTS_DIR = paths["gs_outputs_dir"]
    ...
    if args.gs_configs:
        print(f"[INFO] Mode:            EXTERNAL (--gs_configs)")
    else:
        print(f"[INFO] Mode:            cam2sim auto-detection")
    print("=" * 80)
    ...
    if not only_carla:
        if args.gs_configs:
            print("\n[INFO] Using EXTERNAL GS configs (legacy override)")
            split_configs = build_split_configs_from_external(
                gs_configs=args.gs_configs,
                gs_data_root=args.gs_data_root or GS_DATA_ROOT,
                utm_transforms=args.utm_transforms,
                frame_positions_dir=args.gs_data_root or GS_DATA_ROOT,
            )
        else:
            print("\n[INFO] Auto-detecting split models (cam2sim layout)...")
            split_configs = auto_detect_splits_cam2sim(GS_OUTPUTS_DIR, GS_DATA_ROOT)
